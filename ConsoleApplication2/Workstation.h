...
void viewWorkstation() //Calling view function, As declared at the top.
{
    ...
    while (recordcount<maxworkstation && fin >> Myworkstation[recordcount].ID >> Myworkstation[recordcount].CPUcores >> Myworkstation[recordcount].GPUvRAM >> Myworkstation[recordcount].Powersupply) //using loop to take input from the file releated to the specific fields with 4 columns.
    {
...
void updateWorkstation() //Calling update function, As declared at the top.
{
    ...
    while (recordcount<maxworkstation && fin >> Myworkstation[recordcount].ID >> Myworkstation[recordcount].CPUcores >> Myworkstation[recordcount].GPUvRAM >> Myworkstation[recordcount].Powersupply) //reads the data accordingly using the variables from the file.
    {
